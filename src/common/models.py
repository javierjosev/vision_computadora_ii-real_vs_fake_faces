...
class FinalFacesCNN(nn.Module):
    def __init__(self):
        super(FinalFacesCNN, self).__init__()
        ...
        self.fc5 = nn.Linear(128, 1)  # Final output layer

        self.final = torch.nn.Sigmoid()  # For binary classification
    ...
    def forward(self, x):
        ...
        x = self.conv4(x)
        x = self.conv5(x)
        x = x.view(-1, 512 * 7 * 7)  # Adjust for additional layers
        x = self.fc1(x)
        x = self.fc2(x)
        x = self.fc3(x)
        x = self.fc4(x)
